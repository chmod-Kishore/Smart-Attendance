...
const SessionDetails = (props) => {
  ...
  async function getQR() {
    if (!props.currentSession?.[0]) return; // ✅ Prevent calling API if session is undefined
    await axios
  ...
  useEffect(() => {
    getQR();
  }, [props.currentSession]); // ✅ Runs only when currentSession updates

  // ✅ Show a loading message while data is being fetched
  if (!props.currentSession?.[0]) {
    return (
      <div className="popup">
      ...
      <div className="popup-inner">
        <div className="popup-content">
          <div className="session-details">
            ...
            <p>
              <strong>Session Radius</strong>:{" "}
              {props.currentSession?.[0]?.radius} meters
            </p>
          </div>
          ...
          <table>
            ...
            <tbody>
              {props.currentSession?.[0]?.attendance?.map((student, index) => (
                ...
                <tr>
                  <td colSpan="6">No students attended yet</td>
                </tr>
              )}
            </tbody>
          </table>
